...
export class Payment {
    ...
    static async generateFinancialReport(options = {}, userId) {
        try {
            ...
            // Calculate totals - Ensure these are proper numeric values, not strings
            const regRevenue = Number(regResults[0].total_contracted_amount) || 0;
            const sponsorRevenue = Number(sponsorResults[0].total_contracted_amount) || 0;
            const accomRevenue = Number(accomResults[0].total_contracted_amount) || 0;

            const regReceived = Number(regResults[0].total_received_amount) || 0;
            const sponsorReceived = Number(sponsorResults[0].total_received_amount) || 0;
            const accomReceived = Number(accomResults[0].total_received_amount) || 0;
            ...
            // Save report to financial_summaries without transaction
            try {
                const saveQuery = `
                ...
                console.error('Failed to save financial report:', saveError);
                // Continue even if saving fails - we can still return the report
            }
